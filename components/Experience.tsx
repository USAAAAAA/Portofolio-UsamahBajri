...
const experiences = [
  ...
  },
  {
    ...
    type: "Organization",
    images: [
      "/images/experience/karate1.jpg",
      "/images/experience/karate2.jpg",
    ],
    color: "bg-red-600/10",
  },
];
          ...
          {experiences.map((exp, index) => (
            <div key={index} className="py-20 border-b border-white/10">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-3">
                  ...
                  <ul className="space-y-4">
                    {exp.description.map((desc, i) => (
                      <li
                        key={i}
                        className="flex gap-4 text-gray-400 text-sm leading-relaxed"
                      >
                ...
                <div
                  className={`lg:col-span-4 grid ${exp.layout === "grid" ? "grid-cols-2 gap-4" : "grid-cols-1"}`}
                >
                  {/* Foto Pertama */}
                  <div
                    className={`relative rounded-2xl overflow-hidden border border-white/5 shadow-2xl 
    ${exp.layout === "grid" ? "aspect-3/4" : "aspect-8/4"}`}
                  >
                    <Image
                      src={exp.images[0]}
                      alt={`${exp.title} 1`}
                  ...
                  {/* Foto Kedua */}
                  <div
                    className={`relative rounded-2xl overflow-hidden border border-white/5 shadow-2xl mt-8 
    ${exp.layout === "grid" ? "aspect-3/4" : "aspect-8/4"}`}
                  >
                    <Image
                      src={exp.images[1]}
